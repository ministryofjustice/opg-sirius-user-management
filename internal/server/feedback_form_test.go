...
func TestGetFeedbackFormReturnsPermissionsErrorsIfInputTooLong(t *testing.T) {
	assert := assert.New(t)
	...
	w := httptest.NewRecorder()
	r, _ := http.NewRequest("POST", "/feedback-form", strings.NewReader(""))

	handler := feedbackForm(client, template)
	err := handler(sirius.PermissionSet{}, w, r)

	assert.Equal(sirius.ValidationError{
		Message: "stringLengthTooLong",
		Errors:  nil,
	}, err)
}

func TestGetFeedbackFormReturnsPermissionsErrorsIfNoFeedback(t *testing.T) {
	assert := assert.New(t)
